...
| --------- | ------------------------------------------ | ------------------------------------------------------------------------------------------- |
| Agent     | Codex, GitHub Copilot, OpenCode, Pi        | ACP executable, model credentials, native configuration, network, and capability behavior.  |
...
The repository generates every built-in Agent × Sandbox case through one smoke runner. The docs do not duplicate that matrix or its pinned tool versions: inspect the current [`smoke-matrix.ts`](https://github.com/we-are-singular/aml/blob/main/sdk/tests/smoke/smoke-matrix.ts) when the exact setup matters.
