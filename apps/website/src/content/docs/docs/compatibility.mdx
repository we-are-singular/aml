...
The repository generates every built-in Agent × Sandbox case through one smoke runner. The docs do not duplicate its pinned tool versions: inspect the current [`smoke-config.ts`](https://github.com/we-are-singular/aml/blob/main/sdk/tests/smoke/smoke-config.ts) when the exact setup matters. Docker, Daytona, and Modal cases use `docker.io/wearesingular/aml-agent-sandbox:dev`; Local cases require the same Agent executables on the host.
