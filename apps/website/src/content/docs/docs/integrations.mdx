...
[`dockerSandbox()`](/docs/providers/sandboxes/docker/) expects the image to already contain the compatible ACP executable, Node/runtime dependencies, project dependencies, credentials strategy, and any command-line tools the workflow needs. AML does not build the image or install an Agent.
