...
[`localSandbox()`](/docs/providers/sandboxes/local/) runs trusted host processes. It is appropriate for a developer-owned checkout, not a hostile-code boundary.
...
[`dockerSandbox()`](/docs/providers/sandboxes/docker/) defaults to `wearesingular/aml-agent-sandbox:latest`, AML's portable Agent runtime from its canonical Docker Hub release channel. The repository smoke matrix uses the public, mutable `ghcr.io/we-are-singular/aml-agent-sandbox:dev` nightly/edge build while validating the next release. Applications can override the default with an immutable version, digest, or custom image containing the compatible ACP executable, project dependencies, credentials strategy, and command-line tools their workflow needs. The provider does not build the image or install an Agent during acquisition.
...
[`s3Workspace()`](/docs/providers/workspaces/s3/) needs an S3-compatible client/configuration and storage permissions for its index, revision objects, lock, and cleanup operations.
...
In a real application supply the providers through the runtime or explicit props. [`<File />`](/docs/reference/primitives/file/) is shown at the [`<Workspace />`](/docs/reference/primitives/workspace/) boundary because it persists text through the Workspace provider. Host-backed [`<Agent />`](/docs/reference/primitives/agent/) providers and [`<Script />`](/docs/reference/primitives/script/) can use trusted local execution; place them inside [`<Sandbox />`](/docs/reference/primitives/sandbox/) when their processes must run in that selected environment.
