...
[`dockerSandbox()`](/docs/providers/sandboxes/docker/) defaults to `wearesingular/aml-agent-sandbox:latest`, AML's portable Agent runtime from its canonical Docker Hub release channel. The repository smoke matrix uses `ghcr.io/we-are-singular/aml-agent-sandbox:dev` while validating changes from `main`. Applications can override the default with an immutable version, digest, or custom image containing the compatible ACP executable, project dependencies, credentials strategy, and command-line tools their workflow needs. The provider does not build the image or install an Agent during acquisition.
