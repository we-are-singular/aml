...
This is the canonical three-provider composition: Codex supplies the [`<Agent />`](/docs/reference/primitives/agent/) session, Docker supplies disposable execution, and S3 supplies durable [`<Workspace />`](/docs/reference/primitives/workspace/) revisions.
...
<Aside type="note" title="Credential flow">
  `apiKey` is captured by [`codexAgent()`](/docs/providers/agents/codex/) and mapped to `CODEX_API_KEY` for the ACP
