...
Neither choice makes model input trusted. When a model should receive either capability, grant it lexically to the nearest [`<Agent />`](/docs/reference/primitives/agent/) and constrain grants with runtime allowlists. A JavaScript Tool can instead be called directly by active application component code without any model grant; an MCP descriptor is not an application-call API.
...
| Contract             | Callable returned by `defineTool()`, with input schema and JSON-compatible result | [`defineMcpServer()`](/docs/reference/mcp-server/) transport descriptor; provider negotiates the server |
