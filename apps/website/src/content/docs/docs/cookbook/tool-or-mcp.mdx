...
Neither choice makes model input trusted. Both are granted lexically to the nearest `<Agent />` and can also be constrained by runtime allowlists.
...
| Contract             | `defineTool()` input schema and JSON-compatible result                | `defineMcpServer()` transport descriptor; provider negotiates the server                               |
...
- [`AmlRuntime` allowlists and trace options](https://github.com/we-are-singular/aml/blob/main/sdk/src/core/aml-runtime.ts)
