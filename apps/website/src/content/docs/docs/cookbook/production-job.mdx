...
This is an operational recipe, not a zero-credential demo. It shows the runtime boundary a Node job should own: provider construction, finite limits, redacted tracing, cancellation, and a clear result boundary. The live [`<Agent />`](/docs/reference/primitives/agent/) still needs its executable, model configuration, and credentials.
...
AML does not install the executable, provision a model account, or turn [`localSandbox()`](/docs/providers/sandboxes/local/) into a security boundary.
...
- [`createConsoleTracer()`](/docs/observability/) is a dependency-free console sink, not a metrics backend. Adapt `TraceSink` to your logger or telemetry system if you need durable event aggregation.

## API and source links

- [`AmlRuntime`, `AmlRuntimeOptions`, and `AmlEvaluationOptions`](/docs/reference/runtime/)
- [`createConsoleTracer()` and `TraceSink`](/docs/observability/)
