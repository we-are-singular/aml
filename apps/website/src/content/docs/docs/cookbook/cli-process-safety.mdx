...
[`<Script />`](/docs/reference/primitives/script/) requires a Sandbox with an enforcing runtime. The local provider needs `read-write` because a host process cannot enforce read-only filesystem access.
