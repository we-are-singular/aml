...
<Aside type="note" title="FollowUp is not a dynamic loop">
  You cannot await the outline inside this same `<Agent />` and then decide whether to append `<FollowUp />
  `. Host-side branching requires a separate component-local `evaluate()` boundary targeting a new `<Agent />
  `, after which application code can validate the returned value and author the next `<Agent />` tree.
</Aside>
...
- [`<FollowUp />` implementation](https://github.com/we-are-singular/aml/blob/main/sdk/src/components/follow-up/follow-up.tsx)
