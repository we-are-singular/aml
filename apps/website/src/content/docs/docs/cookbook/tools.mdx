...
Attach one application-owned function to an [`<Agent />`](/docs/reference/primitives/agent/) without turning its return value into prompt text. The workflow injects an application-owned repository into a component, then grants a narrow [`<Tool />`](/docs/reference/primitives/tool/) rather than exposing the repository itself.
