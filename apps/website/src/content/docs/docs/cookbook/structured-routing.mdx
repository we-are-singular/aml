...
1. an Agent returns a schema-validated route record;
...
3. only after those checks does the application construct `localWorkspace()` and run a handoff `<Agent />` inside `<Workspace />`.
...
- [`evaluate` implementation](https://github.com/we-are-singular/aml/blob/main/sdk/src/core/evaluate.ts)
