...
Understand how AML attaches an MCP descriptor to one [`<Agent />`](/docs/reference/primitives/agent/) and keeps that capability scoped. This page intentionally uses the repository's non-networking [`mcp.tsx`](https://github.com/we-are-singular/aml/blob/main/examples/src/capabilities/mcp.tsx) fixture.
...
1. [`defineMcpServer()`](/docs/reference/mcp-server/) validates and freezes a named stdio or Streamable HTTP descriptor. It does not open a connection.
2. [`<Mcp />`](/docs/reference/primitives/mcp/) attaches that exact descriptor to the nearest [`<Agent />`](/docs/reference/primitives/agent/).
