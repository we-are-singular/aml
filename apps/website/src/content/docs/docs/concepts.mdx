...
| run a command or long-lived process in a controlled cwd | Sandbox   | The operation needs execution authority, limits, cancellation, or isolation.   |
...
| JavaScript `<Tool />`               | AML application process                                                                                                  | Calls the function captured by `defineTool()`. It is an application capability, not code moved into the Sandbox.                     |
...
<CardGrid>
  <Card title="Agent" icon="rocket">
    A model-facing session. Child `<Agent />` results become parent input; `<Tool />
    `, `<Mcp />
    `, `<Skill />
    `, and `<FollowUp />` are scoped to the owning `<Agent />
    `.
  </Card>
...
`<Workspace />`, `<Sandbox />`, and `<Context.Provider />` are lexical boundaries. A descendant inherits the active scope while it evaluates, and the boundary releases after its subtree settles.
...
<Aside type="caution" title="Local is trusted execution">
  `localSandbox()` runs host processes. It provides the common Sandbox API but is not a hostile-code isolation boundary.
  Use a provider with an appropriate isolation model for model-controlled or untrusted work.
</Aside>
