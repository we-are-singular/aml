...
| `<Include />`                       | AML application for `src`; nearest active filesystem for `path`                                                          | Decodes inline content as UTF-8, or emits a bounded Agent-visible read instruction without decoding when `maxBytes` is exceeded.                                                            |
