...
| -------------------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------ |
| [`<Agent />`](/docs/reference/primitives/agent/)         | A provider session, prompts, turns, capabilities, and the final result | [Codex, OpenCode, and Pi](/docs/providers/agents/)                 |
...
AML keeps the workflow tree stable while provider factories own vendor behavior: executable discovery, credentials, model names, transport, capability translation, and environment setup. The same `<Agent />` tree can use Codex, OpenCode, or Pi, but their options and prerequisites are not interchangeable. Read the [compatibility guide](/docs/compatibility/) and the relevant [provider guide](/docs/providers/) before treating a combination as supported.
