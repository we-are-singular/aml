...
`<Agent />` may run without an explicit `<Sandbox />` or `<Workspace />` when its provider can do so, but `<Script />`, `<File />`, and provider features that require a resource must have the appropriate active boundary. AML never silently executes `<Script />` on the host because no `<Sandbox />` was configured.
...
| run a command or long-lived process in a controlled cwd | [`<Sandbox />`](/docs/reference/primitives/sandbox/)     | The operation needs execution authority, limits, cancellation, or isolation.   |
...
| `<Script />`                                              | Active Sandbox runtime                                                                                                   | Executes only through `SandboxRuntime.exec()` or `spawn()` with that lease's cwd, access, timeout, and cancellation behavior.                        |
