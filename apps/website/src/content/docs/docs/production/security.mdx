...
AML trace sinks receive redacted events by default. A sink must explicitly set `captureContent` to receive sensitive prompt, Tool, and result fields. Keep it false for normal production telemetry and send only scalar attributes to an approved sink.
