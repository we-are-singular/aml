...
AML composes capabilities; it does not turn arbitrary model output into a trusted program. Security comes from the boundary around the application, the selected Sandbox platform, the Workspace contents, provider credentials, and the effects exposed through Tools and MCP.
