...
AML trace events are immutable and contain scalar attributes, correlation ids, sequence numbers, and timestamps. Span start and end events carry lifecycle boundaries; only span-end events carry `durationMs` and `status`. Point events include `agent.turn`, `capability.tool`, `capability.mcp`, and `loop.transition`.
