...
| `ToolInputError`         | `name === "ToolInputError"`         | Model-supplied [`<Tool />`](/docs/reference/primitives/tool/) input fails JSON or input-schema validation           | Usually ask the Agent to correct its call; retry only with a bounded policy         |
| `ToolOutputError`        | `name === "ToolOutputError"`        | [`<Tool />`](/docs/reference/primitives/tool/) output cannot cross the provider boundary or fails output validation | Fix the Tool output/schema; a network retry may be appropriate only inside the Tool |
...
- `<Tool> is only valid inside <Agent>` or `<Mcp> is only valid inside <Agent>` — move the capability under its owning [`<Agent />`](/docs/reference/primitives/agent/).
- `<Script> requires an enclosing <Sandbox>` — configure an outer [`<Sandbox />`](/docs/reference/primitives/sandbox/); [`<Script />`](/docs/reference/primitives/script/) never falls back to `child_process` on the host.
- `<File> requires an enclosing <Workspace>` — write through [`<Workspace />`](/docs/reference/primitives/workspace/).
