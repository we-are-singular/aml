...
- `an oversized <Include src> requires a containing <Agent>` or `an oversized <Include path> in a host Workspace requires a containing <Agent>` — move the Include into the Agent that will receive and read its staged file reference, or raise/remove `maxBytes`.
