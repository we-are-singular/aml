...
The repository builds `aml-agent-sandbox` from [`images/aml-agent-sandbox/Dockerfile`](https://github.com/we-are-singular/aml/blob/main/images/aml-agent-sandbox/Dockerfile). It includes the AML CLI and SDK, the Agent/ACP packages used by AML's providers, and a practical coding environment. The Dockerfile and package manifests are the source of truth for the exact tools and versions in each release.
...
Stable releases include provenance and an SBOM. The manual release process publishes semantic-version and `latest`
tags to Docker Hub, verifies the published digest, and signs that digest.

Relevant changes on `main` are published separately as `ghcr.io/we-are-singular/aml-agent-sandbox:dev` for repository
development and smoke testing.

| Reference           | Purpose                                                  |
| ------------------- | -------------------------------------------------------- |
| Docker Hub `latest` | Newest stable release and provider-factory default.      |
...
For release or deployment evaluation, set `AML_AGENT_SANDBOX_IMAGE` to the exact Docker Hub semantic version or digest
being validated. Do not treat `latest` or GHCR `dev` as immutable inputs.
