...
That makes it the current recommended **starting image for `opencodeAgent()` with `dockerSandbox()`**. It proves executable-level compatibility, not model credentials, network policy, project dependencies, Docker hardening, or a successful live model session.
