---
title: Alternative and custom Sandbox images
description: Evaluate third-party images, custom image recipes, and compatibility requirements for AML Sandbox providers.
---

import { Aside } from "@astrojs/starlight/components"

AML's own full and single-Agent images, Docker Hub tags, bleeding-edge GHCR channel, usage, and extension model are documented in [AML Agent Sandbox images](/docs/sandbox-images/). Start there unless a third-party image or provider-native recipe is an explicit deployment requirement.
...
For release or deployment evaluation, set `AML_AGENT_SANDBOX_IMAGE` to the exact Docker Hub semantic version or digest being validated. Do not treat `latest` or the [GHCR `dev` channel](/docs/sandbox-images/#bleeding-edge-dev-image) as immutable inputs.
