...
The repository builds `aml-agent-sandbox` from [`images/aml-agent-sandbox/Dockerfile`](https://github.com/we-are-singular/aml/blob/main/images/aml-agent-sandbox/Dockerfile). It uses Debian/glibc and includes Node.js 26, Python 3, Git, common shell/network/archive tools, the AML CLI and SDK, and the Agent/ACP packages verified by AML's smoke matrix.

Docker Hub is the canonical stable registry ([see all versions](/docs/reference/changelog/docker/)):

```text
docker.io/wearesingular/aml-agent-sandbox:latest
```

Stable releases include provenance and an SBOM. The manual release process publishes semantic-version, source-revision,
and `latest` tags to Docker Hub, verifies the published digest, and signs that digest.

GHCR hosts the public development (`dev`) channel, also referred to as the nightly or edge build:

```text
ghcr.io/we-are-singular/aml-agent-sandbox:dev
```

| Reference                 | Purpose                                                                       |
| ------------------------- | ----------------------------------------------------------------------------- |
| Docker Hub `latest`       | Newest validated stable release and provider-factory default.                 |
| Docker Hub `X.Y.Z`        | Immutable semantic version for reproducible deployments.                      |
| Docker Hub `sha-<commit>` | Source-revision reference produced by a manual release.                       |
| GHCR `dev`                | Public mutable build from the latest relevant `main` revision; not a release. |

GitHub Actions replaces `dev` after relevant image changes land on `main`. It may therefore update more than once in a
day or remain unchanged on days without image changes. GHCR does not mirror Docker Hub's stable tags or digests.
...
The image's `aml` command can run a clean mounted TypeScript, TSX, or JavaScript workflow that imports `@aml-jsx/sdk`
without a local install. The packages live outside `/workspace`, and standard ancestor `node_modules` resolution keeps them
visible when Docker replaces `/workspace` with a bind mount. The image deliberately omits credentials, Agent home state,
other project dependencies, cloud CLIs, Docker-in-Docker, browsers, compilers, and additional language runtimes. It runs
as the unprivileged `aml` user by default. AML's local Docker smoke passes the host UID/GID through
[`dockerSandbox({ user })`](/docs/providers/sandboxes/docker/#options) so the process can access its bind-mounted
Workspace. The image also ships its license, third-party notices, and GitHub Copilot CLI redistribution license.

<Aside type="caution" title="Preview tag">
  GHCR `dev` is mutable and is not synchronized with a Docker Hub tag or digest. Do not use it as a production pin. Use
  a Docker Hub semantic version or digest after validating the image with your provider, credentials, network policy,
...
For repository development, run the small image smoke check against the current GHCR `dev` build:

```sh
cd images/aml-agent-sandbox
AML_AGENT_SANDBOX_IMAGE=ghcr.io/we-are-singular/aml-agent-sandbox:dev npm run check
```

For release or deployment evaluation, replace `dev` with the exact Docker Hub semantic version or digest being
validated. Do not treat either `latest` or `dev` as an immutable input.
