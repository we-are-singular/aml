...
The repository builds `aml-agent-sandbox` from [`images/sandbox/Dockerfile`](https://github.com/we-are-singular/aml/blob/main/images/sandbox/Dockerfile). It includes the AML CLI and SDK, the Agent/ACP packages used by AML's providers, and a practical coding environment. The Dockerfile and package manifests are the source of truth for the exact tools and versions in each release.

Docker Hub is the canonical stable registry ([see all versions](/docs/reference/changelog/sandbox/)):
