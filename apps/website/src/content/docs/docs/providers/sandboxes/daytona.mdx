...
Use `read-write` for a Daytona ACP Agent unless the complete Agent/Sandbox contract you selected provides another supported launch path. A read-only Daytona Sandbox is suitable for structural composition or an operation that never invokes the runtime, not for ordinary `<Script />` components or ACP sessions.
