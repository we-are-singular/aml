...
<Aside type="danger" title="Restricted-tier egress blocks some coding agents">
  Daytona's organization-level firewall for restricted tiers cannot be widened by `create.domainAllowList`. GitHub
  Copilot's `*.githubcopilot.com` endpoints are currently absent from Daytona's published essential-services list and
  are reset during TLS, so GitHub Copilot CLI cannot discover models in those Sandboxes. Cline's own `api.cline.bot`
  platform endpoint is affected as well, although Cline configured directly against an allowed model provider may use a
  different route. Amp, Anthropic, Cursor, and Devin domains are present in Daytona's published allowlist, but verify
  the exact Agent endpoint before production use. See [Daytona's network
  limits](https://www.daytona.io/docs/en/network-limits/) and [GitHub's Copilot
  allowlist](https://docs.github.com/en/copilot/reference/copilot-allowlist-reference).
</Aside>
...
| Agent API resets during TLS                            | Daytona's restricted-tier organization firewall does not allow the provider domain.       | Check Daytona's essential-services list; per-Sandbox allowlists cannot widen it. |
