...
Use `read-write` for a Daytona ACP Agent unless the complete Agent/Sandbox contract you selected provides another supported launch path. A read-only Daytona Sandbox can serve live [`<Include path>`](/docs/reference/primitives/include/) reads and writable invocation-private Agent staging, but it cannot run ordinary [`<Script />`](/docs/reference/primitives/script/) components or ACP sessions because those require process execution.
