...
Omitting `image` selects `wearesingular/aml-agent-sandbox:latest`. Pin an immutable version or digest when reproducibility matters, or provide an application image with additional project dependencies.
...
Modal's official OpenCode and Claude Code examples define custom Modal Images rather than publishing reusable registry tags. AML currently resolves registry images through `images.fromRegistry()`. [Recommended Sandbox images](/docs/providers/sandboxes/images/#modal-images) explains that distinction and the supported path for bringing those recipes into AML.
...
7. On writable release, Modal archives the full `/workspace` tree, copies it to the host, and mirrors additions, changes, and deletions into the local materialization.
8. Modal terminates the Sandbox. Reconciliation and termination failures are preserved together when both occur.
...
Use `read-write` for an ACP Agent unless the selected Agent and image have a separate supported launch contract. If you need command execution against a read-only Workspace, Docker’s read-only bind mount has different semantics; do not assume remote transfer providers are interchangeable.
