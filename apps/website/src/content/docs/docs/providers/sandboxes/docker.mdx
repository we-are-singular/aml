...
Docker owns the disposable container and the process transport. The application owns the image, Docker daemon, host configuration, identity, network policy, resource limits, and Agent installation. AML does not build a Dockerfile, install an ACP executable, or infer which Agent an image should run.
...
| ---------------- | -------- | ----------------- | ------------------------------------------------------------------------------------------- |
| `image`          | `string` | Required          | Named image reference passed to `docker run`. Must be a non-empty normalized string.        |
...
`node:26-alpine` is a valid smoke-test image for basic shell/Node commands, but it is not a complete Codex, GitHub Copilot, OpenCode, or Pi Agent image merely because it contains Node. Build and publish a deliberate image for a real Agent workflow, or use the trusted `setup` hook for experiments.

See [Recommended Sandbox images](/docs/providers/sandboxes/images/) for the verified Cloudflare OpenCode image, current Codex build baseline, Modal recipe status, and an image verification checklist.
...
2. Docker starts a named detached container with `--rm`, the selected image, `/workspace` bind mount, logical guest cwd, and a keepalive `sh` process.
...
- rootless mode or a non-root user;
