...
`node:26-alpine` is a valid smoke-test image for basic shell/Node commands, but it is not a complete Codex, OpenCode, or Pi Agent image merely because it contains Node. Build and publish a deliberate image for a real Agent workflow, or use the trusted `setup` hook for experiments.
