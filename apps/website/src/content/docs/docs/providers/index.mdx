...
<CardGrid>
  <Card title="Credential-free learning" icon="approve-check">
    Use `DeterministicAgentProvider` from `@aml-jsx/sdk/testing`. It records normalized calls and avoids network or
    executable setup.
  </Card>
  <Card title="Trusted local automation" icon="laptop">
    Pair an Agent with `localSandbox()` and `localWorkspace()` only when the workflow and every generated command are
    trusted on the host.
  </Card>
  ...
  <Card title="Durable remote workflows" icon="cloud-download">
    Combine Daytona or Modal with `filesystemWorkspace()` or `s3Workspace()` when disposable execution must load and
    publish revision-backed state.
  </Card>
</CardGrid>
