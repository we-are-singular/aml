...
For container execution, [AML Agent Sandbox](/docs/providers/sandboxes/images/#aml-agent-sandbox) includes this Pi,
`pi-acp`, and `pi-mcp-adapter` baseline and is the default image for Docker, Daytona, and Modal. Local Sandbox still
requires the complete chain on the host.
