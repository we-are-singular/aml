...
<Aside type="caution" title="Install it where the Agent process runs">
  A host-global installation is available to `localSandbox()`. Docker, Daytona, Modal, or another remote Sandbox needs
  the executable in its own image or environment. Run `opencode --version` through that same execution path before
  debugging AML configuration.
</Aside>
...
For authored MCP servers, use `<Mcp />` and the `allowedMcpServers` runtime policy. OpenCode receives the resulting server configuration through the shared ACP session. The server's transport, headers, environment, and executable still need to be available in the environment where the provider runs.

## Structured output and follow-ups

`evaluate(value, schema)` uses the shared ACP structured-output path. OpenCode receives the provider session's output request and AML validates the final JSON against the supplied Standard Schema. A structured evaluation must resolve to one `<Agent />` and cannot be satisfied by ordinary prose.

`<FollowUp />` stays in the same OpenCode session. It cannot change the Agent's Tools or MCP set between turns; later turns stop when an earlier turn fails. This makes follow-ups useful for clarification, not for dynamically widening capabilities.
...
3. **A Tool is not visible**
   ...
   Confirm that `<Tool />` is inside `<Agent />`, its name is unique, its input schema is valid JSON Schema, and the runtime `allowedTools` policy does not exclude it. Tool execution remains host-side.
