...
<Aside type="danger" title="Restricted Daytona tiers currently block GitHub Copilot">
  Daytona's organization-level firewall for restricted tiers takes precedence over per-Sandbox `domainAllowList`
  settings. GitHub Copilot CLI requires `*.githubcopilot.com`, which is not in Daytona's published essential-services
  list, so model discovery is reset before an HTTP response is received. Adding the domain through
  `daytonaSandbox({ create: { domainAllowList: ... } })` cannot override that policy. Use another Sandbox, a Daytona
  tier with unrestricted egress, or confirm that Daytona has added the domain before deploying this combination. This
  is a Daytona network failure, not evidence of an invalid Copilot token or exhausted Copilot credits. See
  [Daytona's network limits](https://www.daytona.io/docs/en/network-limits/) and
  [GitHub's Copilot allowlist](https://docs.github.com/en/copilot/reference/copilot-allowlist-reference).
</Aside>
...
The AML repository pins `@github/copilot@1.0.79` for deterministic development and smoke setup. That is the verified
...
```sh title="Terminal"
npm install --global @github/copilot@1.0.79
```
...
3. Authored AML system content becomes an explicit first-turn prelude because ACP has no portable system-message field.
...
6. **Daytona resets the Copilot model endpoint**

   If model discovery fails with `ECONNRESET` against `api.githubcopilot.com` or
   `api.individual.githubcopilot.com`, test TLS from the same Daytona Sandbox. Restricted-tier organization policy
   cannot be widened with a per-Sandbox `domainAllowList`; use an unrestricted environment or ask Daytona to add
   `*.githubcopilot.com` to its essential-services allowlist.
