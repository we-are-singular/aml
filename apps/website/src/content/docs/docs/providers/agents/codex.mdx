...
AML JavaScript Tools are exposed through the shared ACP MCP bridge when the Agent is running with a compatible ACP session. MCP servers declared in the AML tree are passed through the shared ACP session engine after allowlist and capability checks. These are Agent capabilities, not a replacement for Sandbox isolation.

## Structured output

When `evaluate(value, schema)` is used, AML exposes `aml_submit_result` through the shared ACP MCP bridge. Each submitted candidate is immediately validated against the supplied Standard Schema. Codex receives a tool error for an invalid candidate and may correct it in the same turn. AML accepts the first valid candidate and ignores later submissions.

```tsx
import { Agent, evaluate, codexAgent } from "@aml-jsx/sdk"
import { z } from "zod"
...
const finding = await evaluate(
  <Agent provider={codexAgent({ model: "gpt-5.3-codex" })}>Inspect the change and return one concise finding.</Agent>,
  Finding
)
```

The turn must submit at least one valid JSON result for the schema. A prose-only answer or a turn that ends without a valid submission rejects the evaluation. With tracing enabled, each attempt appears as an `agent.output` event.
...
5. **Structured output fails**

   Enable tracing and inspect `agent.output` events. Invalid candidates are returned to Codex for correction; a missing submission or a turn that finishes without a valid candidate is an evaluation error.
