...
When `evaluate(value, schema)` is used, AML asks the provider session for structured output and validates the returned JSON against the supplied Standard Schema. The Codex profile contributes the Codex configuration and uses the shared ACP structured-output path; it does not perform a second schema validation itself.
...
The final result must be valid JSON for the schema. A prose answer, invalid JSON, or a provider session that cannot honor structured output rejects the evaluation.
...
5. **Structured output fails**
   ...
   Reduce the schema and inspect the provider's raw model behavior. The schema applies to the final `<Agent />` result; invalid JSON or a missing structured response is an evaluation error.
