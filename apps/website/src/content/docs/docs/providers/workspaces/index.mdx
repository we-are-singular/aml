...
[`<File />`](/docs/reference/primitives/file/) writes into the active materialization when it is placed directly under [`<Workspace />`](/docs/reference/primitives/workspace/), as below. A File inside a Sandbox writes that live guest instead. A Workspace write is not a durable save by itself; enable the save policy if the file must become a revision.
