...
| ---------------------------------------- | ------------------------------- | --------------------------------- | --------- | -------------------------------------------------------------------- |
| [`localWorkspace()`](./local/)           | The configured directory itself | Existing local directory          | No        | Trusted, same-host development and direct file workflows.            |
| [`filesystemWorkspace()`](./filesystem/) | A unique temporary directory    | Local archive or folder artifacts | Yes       | Local jobs that need load, retention, rollback, or revision history. |
| [`s3Workspace()`](./s3/)                 | A unique temporary directory    | S3 object storage                 | Yes       | Distributed workers and durable remote state shared across hosts.    |
...
`<File />` writes into the active materialization and therefore requires `<Workspace />`. It is not a durable save by itself; enable the `<Workspace />` save policy if the file must become a revision.
...
For provider authors, the stable contracts are [`WorkspaceProvider`](https://github.com/we-are-singular/aml/blob/main/sdk/src/components/workspace/workspace-provider.ts), [`WorkspacePersistence`](https://github.com/we-are-singular/aml/blob/main/sdk/src/workspace-persistence/workspace-persistence.ts), and the Workspace section of [`SPEC.md`](https://github.com/we-are-singular/aml/blob/main/SPEC.md#14-workspace). Provider-specific behavior is summarized in [`PROVIDERS.md`](https://github.com/we-are-singular/aml/blob/main/PROVIDERS.md).
