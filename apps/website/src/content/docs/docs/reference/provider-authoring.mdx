...
Use `defineAcpAgentProvider()` when the vendor already exposes an Agent Client Protocol executable. AML's shared ACP adapter owns process launch, one-session turn order, JavaScript Tool and structured-output bridging, MCP descriptors, temporary state, local versus Sandbox execution, abort, and cleanup.
