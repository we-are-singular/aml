...
That default comes from AML's canonical stable releases on Docker Hub. The repository smoke matrix instead follows the
public, mutable `ghcr.io/we-are-singular/aml-agent-sandbox:dev` nightly/edge channel.
...
The image supplies software; the provider supplies process transport, Workspace mapping, cancellation, and cleanup;
the application still supplies credentials, network and resource policy, project dependencies, and deployment
hardening. Read [Sandbox images](/docs/sandbox-images/) for the mental model and
[Recommended Sandbox images](/docs/providers/sandboxes/images/) for exact contents and pinning guidance.
