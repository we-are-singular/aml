...
`<System />` routes resolved child text into the system channel of its nearest containing [`<Agent />`](/docs/reference/primitives/agent/). It does not open a provider session itself.
...
Use [`<Include />`](/docs/reference/primitives/include/) for reusable instruction files, [`<Skill />`](/docs/reference/primitives/skill/) for a complete Agent Skills package, and [`<FollowUp />`](/docs/reference/primitives/follow-up/) for later user turns in the same session. A Skill is session-wide and must be declared in the Agent plan rather than inside `<System />`.
