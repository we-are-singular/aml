...
`<Mcp />` grants one Model Context Protocol server to its nearest containing [`<Agent />`](../agent/). Select exactly one form: a provider-native server name or a transport descriptor returned by `defineMcpServer()`.
...
See [Connect an MCP server](/docs/cookbook/mcp/) and [Tool or MCP?](/docs/cookbook/tool-or-mcp/) for complete examples and trade-offs.
