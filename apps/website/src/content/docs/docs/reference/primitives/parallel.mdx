...
Branch output is isolated while work is active. Completion order never changes the rendered result order, and AML does not insert separators between branch strings. Use [`<Block />`](/docs/reference/primitives/block/) when each branch should remain a distinct Markdown section:
