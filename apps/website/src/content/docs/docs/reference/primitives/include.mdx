...
AML caches up to 64 inspected file revisions per runtime. The revision key includes the filesystem, path, byte size, and modification time, so a changed file is inspected again. Size and line count are retained for every cached revision; UTF-8 content is retained only through 256 KiB. Oversized output reports the authored path, size, and line count before the Agent-visible read instruction.
