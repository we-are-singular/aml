...
AML caches up to 64 inspected file revisions per runtime. The revision key includes the filesystem, path, byte size, and modification time, so a changed file is inspected again. `maxBytes` is evaluated independently for every authored Include: it decides whether that request inlines content or emits a read instruction. The cache separately retains size and line count for every revision and keeps UTF-8 content only through 256 KiB. A metadata-only entry is promoted when a later request needs inline content; a later smaller `maxBytes` still emits a read instruction even when that content is cached. AML validates and counts a large host or read-write Sandbox file as a stream instead of retaining its complete body. A read-only Sandbox uses its provider's complete-file API because some providers intentionally prohibit all process execution when they cannot enforce read-only access. Oversized output reports the authored path, size, and line count before the Agent-visible read instruction.
