...
| `maxStateTransitions` |            `16` | `<Loop />` transitions need a workflow budget.                       |
...
- Set `maxStateTransitions` for every workflow that uses `<Loop />`; an unbounded loop is not a retry policy.
