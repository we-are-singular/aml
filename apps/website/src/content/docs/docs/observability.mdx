...
The console tracer omits `agent_message_chunk` and `tool_call_update` events to keep the interactive tree compact, even when content capture is enabled. These events remain in the trace stream for custom sinks. The initial `tool_call` remains visible and includes `toolName` when the ACP Agent supplies its optional programmatic name.
