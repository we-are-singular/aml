...
<Aside type="caution" title="Workflow files are trusted code">
  ...
  execution. An unsandboxed <code>&lt;Script /&gt;</code> also runs as a trusted host process from the CLI working
  directory; its optional relative <code>cwd</code> resolves from that directory. The CLI does not sandbox top-level
