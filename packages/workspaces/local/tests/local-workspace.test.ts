...
import { workspaceProviderConformance } from "@aml/sdk/testing"
...
describe("localWorkspace()", () => {
  it("constructs lazily and validates configured options", async () => {
    ...
    const missing = path.join(
      tmpdir(),
      `aml-missing-${crypto.randomUUID()}`,
    )
    const provider = localWorkspace({ directory: missing })

    expect(provider.name).toBe("local")
    await expect(
      provider.acquire(createRequest("missing")),
    ).rejects.toThrow("cannot be materialized")

    let directoryReads = 0
    const capturedDirectory = path.join(
      tmpdir(),
      `aml-captured-${crypto.randomUUID()}`,
    )
    ...
    await expect(
      capturedProvider.acquire(createRequest("captured")),
    ).rejects.toThrow(capturedDirectory)
    expect(() =>
      localWorkspace({
  ...
  it("materializes the physical directory and persists direct writes", async () => {
    ...
    await writeFile(
      path.join(first.directory, "finding.txt"),
      "persisted finding",
    )
    await first.save()
    await first.release()
    ...
    await expect(
      readFile(
        path.join(second.directory, "finding.txt"),
        "utf8",
      ),
    ).resolves.toBe("persisted finding")
    await second.save()
    await second.release()
    await expect(stat(`${directory}.lock`)).rejects.toHaveProperty(
      "code",
      "ENOENT",
    )
  })
  ...
  it("rejects competing providers for one physical directory", async () => {
    ...
    const first = await firstProvider.acquire(
      createRequest("first-logical-id"),
    )
    const conflict = await secondProvider
      .acquire(createRequest("second-logical-id"))
    ...
    const second = await secondProvider.acquire(
      createRequest("second-logical-id"),
    )
    await second.release()
  })
  ...
  it("enforces contention across processes and canonical paths", async () => {
    ...
    const child = spawn(
      ...
      [
        ...
        path.join(
          import.meta.dirname,
          "fixtures/hold-local-workspace.ts",
        ),
        link,
      ],
      {
    ...
    try {
      ...
      const conflict = await provider
        .acquire(createRequest("parent-contender"))
        .catch((error: unknown) => error)
      ...
      const reacquired = await provider.acquire(
        createRequest("parent-contender"),
      )
  ...
  it("preserves cancellation before filesystem or lock acquisition", async () => {
    ...
    await expect(
      localWorkspace({ directory }).acquire(
        createRequest("cancelled", controller.signal),
      ),
    ).rejects.toBe(reason)
    await expect(stat(`${directory}.lock`)).rejects.toHaveProperty(
      "code",
...
async function createTemporaryDirectory(): Promise<string> {
  const directory = await mkdtemp(
    path.join(tmpdir(), "aml-workspace-local-"),
  )
...
function waitForChildText(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          `Timed out waiting for Local Workspace child "${expected}": ${stderr}`,
        ),
      )
    }, 10_000)
    ...
    const onExit = (code: number | null) => {
      ...
      reject(
        new Error(
          `Local Workspace child exited with ${code}: ${stderr}`,
        ),
      )
    }
    const cleanup = () => {
