...
import type { WorkspaceProvider } from "@aml/sdk"
...
if (
  ...
  throw new Error(
    "Local Workspace exports do not match the reviewed dist-only contract",
  )
}
...
if (
  ...
  packageJson.devDependencies["@types/proper-lockfile"] ===
    undefined
) {
  throw new Error(
    "Local Workspace must own its SDK, lock runtime, and private lock types",
  )
}

const entry = fileURLToPath(
  import.meta.resolve("@aml/workspace-local"),
)
...
)) as BuiltLocalWorkspacePackage
const directory = mkdtempSync(
  path.join(tmpdir(), "aml-local-package-check-"),
)
...
try {
  ...
  if (provider.name !== "local") {
    throw new Error(
      "Built Local Workspace failed its provider lifecycle contract",
    )
  }
} finally {
...
for (const expectedFile of ["dist/index.d.ts", "dist/index.js"]) {
  if (!packedFiles.has(expectedFile)) {
    throw new Error(
      `Local Workspace package is missing ${expectedFile}`,
    )
  }
}
...
console.log(
  "Local Workspace dist runtime, lifecycle, exports, and package are valid",
)
