...
Slice 15 adds a runtime-owned Hookable event layer. Applications subscribe with `runtime.on()` or `runtime.once()`, and providers receive the same subscriber-only surface scoped to their evaluation through `AgentExecutionContext.events`. Immutable provider-neutral trace events, content redaction, and the dependency-free console tree use that same layer; the shared example runner attaches `createConsoleTracer()` to every example.
