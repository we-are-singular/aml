...
export class ModelSchema<Output> {
  ...
  constructor(schema: AmlModelSchema<unknown, Output>) {
    ...
    if (
      ...
      throw new TypeError(
        "Agent output JSON Schema must be an object",
      )
    }
  ...
  async validate(value: unknown): Promise<Output> {
    ...
    const result: SchemaValidation =
      await this.#adapter.validate(captured)

    if (!result.success) {
      throw new TypeError(
        "Agent structured output failed schema validation",
        { cause: result.issues },
      )
    }
