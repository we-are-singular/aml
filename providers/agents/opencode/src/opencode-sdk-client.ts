...
const SUPPORTED_CAPABILITY_SERVER_VERSIONS = new Set(["1.18.4", "1.18.5"])
...
export class OpenCodeSdkClient implements OpenCodeSessionClient {
  ...
  async attachCapabilities(
    ...
    if (input.tools.length === 0 && input.mcpServers.length === 0 && !input.structuredOutput) {
      return new OpenCodeCapabilityAttachment(enabled, async () => undefined)
    }
    ...
    const hostTools = input.tools.filter(tool => tool.kind === "host")
    const javaScriptTools = input.tools.filter(tool => tool.kind === "javascript")
    ...
    const cleanup = async () => {
      ...
      if (errors.length > 1) {
        throw new AggregateError(errors, "OpenCode capability cleanup failed")
      }
    }
    ...
    try {
      ...
      for (const server of input.mcpServers) {
        ...
        const normalized = OpenCodeSdkClient.#permissionCanonical(OpenCodeSdkClient.#sanitizeToolName(name))

        if (normalizedServerNames.has(normalized)) {
          throw new TypeError("OpenCode MCP server names collide after provider normalization")
        }

        for (const existing of normalizedServerNames.keys()) {
          if (normalized.startsWith(`${existing}_`) || existing.startsWith(`${normalized}_`)) {
            throw new TypeError("OpenCode MCP server names overlap after provider normalization")
          }
        }
      ...
      if (javaScriptTools.length > 0) {
        ...
        for (const tool of javaScriptTools) {
          if (tool.inputSchema.type !== "object") {
            throw new TypeError(`OpenCode Tool "${tool.name}" requires an object input schema`)
          }

          const sanitized = OpenCodeSdkClient.#sanitizeToolName(tool.name)

          if (sanitizedNames.has(sanitized)) {
            throw new TypeError("OpenCode JavaScript Tool names collide after provider normalization")
          }
        ...
        bridge = new OpenCodeToolBridge(javaScriptTools, input.context)
        ...
        const status = typeof data === "object" && data !== null ? Reflect.get(data, connection.name) : undefined

        if (typeof status !== "object" || status === null || Reflect.get(status, "status") !== "connected") {
          throw new Error(`OpenCode did not connect AML Tool bridge ${connection.name}`)
        }
      }

      for (const server of input.mcpServers) {
        await this.#attachMcp(server, input.directory, signal, attemptedName => {
      ...
      const availableHostTools =
        input.mcpServers.length > 0 || hostTools.length > 0 || input.structuredOutput
          ? await this.#toolIds(input.directory, signal)
          : new Set<string>()

      if (input.structuredOutput) {
        const reservedName = OpenCodeSdkClient.#permissionCanonical("StructuredOutput")
        const declaredCollision = hostTools.find(
          tool => OpenCodeSdkClient.#permissionCanonical(tool.name) === reservedName
        )
        const ambientCollision = [...availableHostTools].find(
          toolId => OpenCodeSdkClient.#permissionCanonical(toolId) === reservedName
        )
      ...
      if (input.mcpServers.length > 0 || hostTools.length > 0) {
        ...
        for (const server of input.mcpServers) {
          ...
          enabled[`${OpenCodeSdkClient.#sanitizeToolName(name)}_*`] = true
        }
      }
      ...
      if (hostTools.length > 0) {
        for (const tool of hostTools) {
          if (!availableHostTools.has(tool.name)) {
            throw new Error(`OpenCode host Tool "${tool.name}" is unavailable`)
          }
          ...
          if (/[*?]/u.test(tool.name)) {
            throw new Error(`OpenCode host Tool "${tool.name}" contains wildcard syntax`)
          }
      ...
      if (errors.length > 1) {
        throw new AggregateError(errors, "OpenCode capability setup and cleanup failed")
      }
  ...
  async #assertCapabilityNamespaceIsolation(
    ...
    const declaredNames = new Set(
      servers.map(server => (server.kind === "named" ? server.name : server.definition.name))
    )
    const declaredNamespaces = new Map(
      [...declaredNames].map(name => [
        name,
        OpenCodeSdkClient.#permissionCanonical(OpenCodeSdkClient.#sanitizeToolName(name)),
      ])
    )
    ...
    for (const configuredName of Object.keys(statuses)) {
      ...
      const configuredNamespace = OpenCodeSdkClient.#permissionCanonical(
      ...
      for (const [declaredName, declaredNamespace] of declaredNamespaces) {
        if (
          ...
          throw new Error(`OpenCode MCP server "${declaredName}" overlaps undeclared server "${configuredName}"`)
        }
      }
    }
    ...
    for (const toolId of declaredHostToolIds) {
      for (const configuredName of Object.keys(statuses)) {
        ...
        const canonicalToolId = OpenCodeSdkClient.#permissionCanonical(toolId)

        if (canonicalToolId.startsWith(`${configuredNamespace}_`)) {
          throw new Error(`OpenCode host Tool "${toolId}" overlaps MCP server "${configuredName}"`)
        }
      }
    }
    ...
    for (const toolId of hostToolIds) {
      ...
      for (const [declaredName, declaredNamespace] of declaredNamespaces) {
        if (canonicalToolId.startsWith(`${declaredNamespace}_`)) {
          throw new Error(`OpenCode MCP server "${declaredName}" overlaps undeclared host Tool "${toolId}"`)
        }
      }
    }
    ...
    for (const declaredToolId of declaredHostToolIds) {
      const canonicalDeclared = OpenCodeSdkClient.#permissionCanonical(declaredToolId)
      const equivalents = [...hostToolIds].filter(
        toolId => OpenCodeSdkClient.#permissionCanonical(toolId) === canonicalDeclared
      )

      if (equivalents.length > 1 || (equivalents.length === 1 && equivalents[0] !== declaredToolId)) {
        throw new Error(`OpenCode host Tool "${declaredToolId}" has permission-equivalent provider Tool IDs`)
      }
    }
  }
  ...
  async #attachMcp(
    ...
    if (server.kind === "named") {
      if (!Object.hasOwn(existing, name)) {
        throw new Error(`OpenCode named MCP server "${name}" is unavailable`)
      }
      ...
      if (data !== true) {
        throw new Error(`OpenCode did not connect named MCP server "${name}"`)
      }
    } else {
      if (Object.hasOwn(existing, name)) {
        throw new Error(`OpenCode configured MCP server "${name}" conflicts with provider configuration`)
      }
      ...
      if (transport.type === "stdio" && transport.cwd !== undefined) {
        throw new Error(`OpenCode does not support cwd for MCP server "${name}"`)
      }
      ...
      const config =
        transport.type === "stdio"
          ? {
              ...
              enabled: true,
              ...(transport.env === undefined ? {} : { environment: { ...transport.env } }),
              type: "local" as const,
            }
          : {
              enabled: true,
      ...
      const status = typeof data === "object" && data !== null ? Reflect.get(data, name) : undefined

      if (typeof status !== "object" || status === null || Reflect.get(status, "status") !== "connected") {
        throw new Error(`OpenCode did not connect configured MCP server "${name}"`)
      }
    }
    ...
    if (typeof status !== "object" || status === null || Reflect.get(status, "status") !== "connected") {
      throw new Error(`OpenCode MCP server "${name}" is not connected`)
    }
   ...
   */
  async #mcpStatus(directory: string | undefined, signal: AbortSignal): Promise<Readonly<Record<string, unknown>>> {
    const { data } = await this.#client.mcp.status(directory === undefined ? {} : { directory }, {
      signal,
      throwOnError: true,
    })
   ...
   */
  async #disconnectMcp(name: string, directory: string | undefined): Promise<void> {
    ...
    if (data !== true) {
      throw new Error(`OpenCode did not disconnect MCP server "${name}"`)
    }
  }
  ...
  async prompt(input: OpenCodeSessionPromptInput, signal: AbortSignal): Promise<OpenCodeSessionPromptResult> {
    ...
    const structured = hasStructured ? Reflect.get(info, "structured") : undefined
  ...
  async #assertCapabilityServerVersion(signal: AbortSignal): Promise<void> {
    ...
    if (!SUPPORTED_CAPABILITY_SERVER_VERSIONS.has(version)) {
      throw new Error(`OpenCode server ${version} is unsupported for capability isolation`)
    }
  }
   ...
   */
  async #toolIds(directory: string | undefined, signal: AbortSignal): Promise<ReadonlySet<string>> {
    // Capture the registry response as unknown because provider/plugin output
    // can violate generated SDK types at runtime.
    const { data } = await this.#client.tool.ids(directory === undefined ? {} : { directory }, {
      signal,
      throwOnError: true,
    })
